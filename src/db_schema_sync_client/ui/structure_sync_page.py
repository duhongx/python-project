...
from db_schema_sync_client.ui.config_dialog import ProfileManagerDialog
...
class StructureSyncPage(QWidget):
    ...
    def _load_source_metadata(self) -> None:
        ...
        if profile is None:
            self._show_status_message("请先选择源端连接", 3000)
            return
        ...
        self._source_snapshot = None
        self._show_progress()
        self._show_status_message("正在加载源端结构…")
    ...
    def _load_target_metadata(self) -> None:
        ...
        if profile is None:
            self._show_status_message("请先选择目标端连接", 3000)
            return
        password = self._get_password_for_profile(profile)
        if password is None:
            return
        self._target_snapshot = None
        self._show_progress()
        self._show_status_message("正在加载目标端结构…")
    ...
    def _refresh_target_after_sync_and_recompare(self) -> None:
        ...
        self._show_progress()
        self._show_status_message("同步完成，正在刷新目标端结构并重新比对…")
    ...
    def _run_comparison(self) -> None:
        ...
        def _do_compare() -> None:
            ...
            self._show_status_message("正在比对结构…")
            cw.start()
    ...
    def _execute_sync(self, plan: GeneratedSqlPlan) -> None:
        ...
        self._show_progress()
        self._show_status_message(f"正在执行同步（共 {len(plan.statements)} 条 SQL）…")
